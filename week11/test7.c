...
void main(){
    ...
    int res;
    char buf_wr[] = "123456789";
    char buf_rd[sizeof(buf_wr)];
    buf_rd[0] = '\0';
    int fd = open("/home/rlk/mylinux/week11/data.txt", O_RDWR | O_TRUNC | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
    if(fd < 0){
        printf("error: open file:/data.txt failed.\n");
        exit(EXIT_FAILURE);
    }
